...
Deno.serve(async (req) => {
  try {
    ...
    const existing = await base44.entities.StudyPlanDayCompletions.filter({
      userId: user.id,
      planId,
      dayNumber,
      date: today,
    ...
    if (existing.length > 0) {
      ...
      completion = await base44.entities.StudyPlanDayCompletions.create({
        userId: user.id,
        planId,
        dayNumber,
        completed: true,
    ...
    return Response.json({ success: true, completion });
  } catch (error) {
    console.error('completeStudyPlanDay error:', error);
...
async function recalculateUserStats(base44, userId) {
  try {
    ...
    const readingSessions = await base44.entities.ReadingSessions.filter({ userId });
    const listeningSessions = await base44.entities.ListeningSessions.filter({ userId });
    const planCompletions = await base44.entities.StudyPlanDayCompletions.filter({ userId });
    ...
    let userStats = await base44.entities.UserStats.filter({ userId });
